                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b-2 border-gray-200">
                            <tr>
                                ...
                                <th class="w-40 p-3 text-sm font-semibold tracking-wide text-center">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            if (isset($_POST['bcari'])) {
                                ...
                                $q = "SELECT * FROM tblaporan INNER JOIN tbsiswa ON tblaporan.nis = tbsiswa.nis WHERE tblaporan.namasiswa LIKE '%$nisnama%' AND tbsiswa.kelas LIKE '%$kelas%' AND tbsiswa.jurusan LIKE '%$jurusan%' AND tblaporan.status = 'Selesai'  ORDER BY tblaporan.idlaporan DESC LIMIT $mulai, $halaman";
                            } else {
                                $q = "SELECT * FROM tblaporan INNER JOIN tbsiswa ON tblaporan.nis = tbsiswa.nis WHERE tblaporan.status = 'Selesai' ORDER BY tblaporan.idlaporan DESC LIMIT $mulai, $halaman";
                            }
                                ...
                                <tr class="odd:bg-[#0a088a]/5 even:bg-[#0a088a]/10 hover:bg-gray-50 align-middle">
                                    ...
                                    <td class="p-1.5 whitespace-nowrap">
                                        <div class="flex flex-col items-center justify-center gap-y-2">
                                            <span class="p-1.5 text-xs font-bold uppercase tracking-wider rounded-lg text-green-800 bg-green-400/60 transition-all select-none">Selesai</span>
                                        </div>
                                    </td>
                                </tr>
