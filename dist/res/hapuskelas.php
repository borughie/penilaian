                        <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="modalHapusKelas<?= $data['idkelas'] ?>" tabindex="-1" aria-modal="true" role="dialog">
                            <div class="modal-dialog modal-dialog-centered relative w-auto pointer-events-none">
                                <div class="modal-content border-none relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                                    ...
                                    <form method="POST" action="./res/aksi_crud.php">
                                        <input type="hidden" name="tidkelas" value="<?= $data['idkelas'] ?>">
                                        <div class="modal-body relative">
                                            <div class="px-4 pb-3 sm:p-6">
                                                <h5 class="text-center font-semibold text-slate-900">Apakah anda yakin akan menghapus data ini? <br>
                                                    <input type="text" class="hidden" name="idkelas" value="<?= $data['idkelas'] ?>">
                                                    <span class="font-extrabold text-transparent text-lg bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600"><?= $data['namakelas'] ?></span>
                                                </h5>
                                            </div>
                                            <div class="px-4 pb-5 text-center mx-auto sm:px-6 rounded-b-md">
                                                ...
                                                <button type="submit" class="inline-flex justify-center rounded-md border border-transparent bg-red-600/80 hover:bg-red-700/80 py-2 w-32 text-sm font-semibold text-white shadow-sm transition-all" name="bhapuskelas">Hapus</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
